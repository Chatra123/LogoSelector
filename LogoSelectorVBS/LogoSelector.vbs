...
'ファイルを直接指定              '大文字で比較
'---------------------------
    If 0 < InStr(1, channel, "CBC", 1) Then              '半角
	    logoPath    = fso.BuildPath(LogoDirPath, "ＣＢＣ.ＣＢＣテレビ.2014.lgd")
	    paramPath   = fso.BuildPath(LogoDirPath, "ＣＢＣ.ＣＢＣテレビ.2014.lgd.autoTune.param")
    ElseIf 0 < InStr(1, channel, "ＣＢＣ", 1) Then       '全角
	    logoPath    = fso.BuildPath(LogoDirPath, "ＣＢＣ.ＣＢＣテレビ.2014.lgd")
	    paramPath   = fso.BuildPath(LogoDirPath, "ＣＢＣ.ＣＢＣテレビ.2014.lgd.autoTune.param")
    ElseIf 0 < InStr(1, channel, "NHK", 1) Then          '半角
        Quit_RequestAbort()
    ElseIf 0 < InStr(1, channel, "ＮＨＫ", 1) Then       '全角
        Quit_RequestAbort()
'    ElseIf 0 < InStr(1, channel, "BS", 1) Then
'        comment = "Disable_MidPrc"                      '追加コメント
'    ElseIf 0 < InStr(1, channel, "ＢＳ", 1) Then
'        comment = "Disable_MidPrc"
    End If
...
'パラメータ　あり
    Sub Quit_withParam()
        WScript.Echo logoPath
        WScript.Echo paramPath
        WScript.Echo comment    '"Disable_MidPrc"
        WScript.Quit(0)
    End Sub
